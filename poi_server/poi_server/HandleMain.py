...
def merge_search_result_and_poi_result(search_result_json, str_search_query_word):
    """
    合并搜索结果 和 poi请求结果
    :param search_result_json:  搜索服务返回结果
    :param str_search_query_word: 请求搜索服务关键字
    """
    try:
...
def handle_compare_request(req_params_map, str_req_path):
    ...
    try:
        ...
        if ((is_full_chinese and len(str_search_query_word) >= 2) or (
            ...
            # 需要查询 POI
            merge_search_result_and_poi_result(search_result_json, str_search_query_word)
        else:
            search_result_json['is_complete'] = True

        str_search_server_rsp_content = json.dumps(search_result_json)
        l_merge_end_time = time.time()
        logger.info("handle request:%s\t cost: %f ms ")%(json.dumps(req_params_map,l_merge_end_time-l_start_time));
        return str_search_server_rsp_content
